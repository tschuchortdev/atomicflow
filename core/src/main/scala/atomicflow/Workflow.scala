...
/** A workflow definition: the identifying/descriptive fields plus the executable
  * `body` and its codec. There is deliberately no separate meta wrapper; every
  * consumer that needs the definition has the executable object (see
  * `spec/core-types.md`).
  *
  * `Cacheable[In]` is captured at construction (the companion's `[In: Cacheable]`
  * context bound); the output codec and throwable codec are resolved at the run
  * call sites, which require them as context parameters.
  */
final class Workflow[In, Out] private[atomicflow] (
  ...
  def create(instanceKey: WorkflowInstanceKey, in: In)(using
      ...
      cacheable: Cacheable[In]
  ): WorkflowInstance[In, Out] =
    runtime.createWorkflowInstance(this, instanceKey, in)
  ...
  def run(instanceKey: WorkflowInstanceKey)(using
      runtime: WorkflowRuntime,
      cacheableIn: Cacheable[In],
      cacheableOut: Cacheable[Out],
...
object Workflow {
  def apply[In: Cacheable, Out](
    ...
    new Workflow[In, Out](id, version, name, description, body, onUnconsumedSignals, summon[Cacheable[In]])
}
